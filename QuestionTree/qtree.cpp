...
void QuestionTree::Distinguish::replaceChild(Node* orig, Node* newChild) {
  ...
  if(this->yesSubtree==orig){
    this->yesSubtree=newChild;
  }
  else if(this->noSubtree==orig)
  {
    this->noSubtree = newChild;
  }
  else
    {throw new std::exception();}
}
...
void QuestionTree::FinalGuess::expand() {
  ...
  if (answerForNew!=true)
  {
    newQuestion = new Distinguish(question,this,newGuess,tree );
  }
  else
  {
    newQuestion = new Distinguish(question,newGuess,this,tree );
  }
    this->parent->replaceChild(this, newQuestion);
    this->parent = newQuestion;
    newGuess->parent = newQuestion;
}
